...
CASE("pixel-center on image-sensor-plane")
{
        ...
        c = mliApCam_get_pixel_center_on_image_sensor_plane(
...
CASE("object-point")
{
        ...
        double object_distance = mli_object_given_focal_and_image(
                focal_length,
                fabs(image_sensor_distance));
        struct mliVec obj =  mliApCam_get_object_point(
                focal_length,
                mliVec_set(0.1, 0.1, image_sensor_distance));
