...
CASE("mliScenery, mliMesh_malloc_from_object_file")
{
        ...
        CHECK(mliMesh_malloc_from_object_file(
                "tests/resources/diff_cube_sphere.off", &diff_cube_sphere));
        ...
        scenery.spherical_cap_hex_T[0].rotation =
                mliQuaternion_set_tait_bryan(0., 0., 0.);
        ...
        scenery.bicircleplanes_T[0].rotation =
        ...
        mliScenery_read_from_path(
                &scenery_back, "tests/resources/scn1.mli.tmp");

        CHECK(scenery.resources.num_functions ==
              scenery_back.resources.num_functions);
...
CASE("scenery intersection interface -> sphere")
{
        ...
        sphere_idx = scenery.num_triangles + scenery.num_spherical_cap_hex;

        hit = mliScenery_intersection(
                &scenery,
                mliRay_set(mliVec_set(0., 0., -10.), mliVec_set(0., 0., 1.)),
                sphere_idx,
                &isec);
        ...
        CHECK(mliVec_equal_margin(
                isec.position, mliVec_set(0., 0., -sphere_radius), 1e-6));
        CHECK(mliVec_equal_margin(
                isec.surface_normal, mliVec_set(0., 0., -1), 1e-6));
...
CASE("struct mliScenery asymetric")
{
        ...
        for (i = 0; i < scenery.num_spheres; i++) {
                const double phi =
                        2. * MLI_PI * (double)(i) / scenery.num_spheres;
                ...
                scenery.spheres_T[i].translation = mliVec_set(
                        5 * cos(phi) + offset.x,
                        -5 * sin(phi) + offset.y,
                        5 * zoff + offset.z);
                scenery.spheres_T[i].rotation =
                        mliQuaternion_set_tait_bryan(0., 0., 0.);
        }
