...
#ifndef MERLICT_MLITRIANGLE_OBB_H_
#define MERLICT_MLITRIANGLE_OBB_H_
...
#define MLI_SIGN3( A ) \
  (((A).x < MLI_TRI_EPS) ? 4 : 0 | ((A).x > -MLI_TRI_EPS) ? 32 : 0 | \
   ((A).y < MLI_TRI_EPS) ? 2 : 0 | ((A).y > -MLI_TRI_EPS) ? 16 : 0 | \
   ((A).z < MLI_TRI_EPS) ? 1 : 0 | ((A).z > -MLI_TRI_EPS) ? 8 : 0)
...
int64_t __mli_check_line(struct mliVec p1, struct mliVec p2, int64_t outcode_diff) {
   ...
   if ((0x02 & outcode_diff) != 0)
      if (__mli_check_point(p1,p2,(-0.5f-p1.x)/(p2.x-p1.x),0x3d) == MLI_INSIDE)
        return(MLI_INSIDE);
   if ((0x04 & outcode_diff) != 0)
      if (__mli_check_point(p1,p2,( 0.5f-p1.y)/(p2.y-p1.y),0x3b) == MLI_INSIDE)
        return(MLI_INSIDE);
   if ((0x08 & outcode_diff) != 0)
      if (__mli_check_point(p1,p2,(-0.5f-p1.y)/(p2.y-p1.y),0x37) == MLI_INSIDE)
        return(MLI_INSIDE);
   if ((0x10 & outcode_diff) != 0)
      if (__mli_check_point(p1,p2,( 0.5f-p1.z)/(p2.z-p1.z),0x2f) == MLI_INSIDE)
        return(MLI_INSIDE);
   if ((0x20 & outcode_diff) != 0)
      if (__mli_check_point(p1,p2,(-0.5f-p1.z)/(p2.z-p1.z),0x1f) == MLI_INSIDE)
        return(MLI_INSIDE);
   return(MLI_OUTSIDE);
}
...
/* If P is outside triangle bbox, there cannot be an intersection. */
   ...
   if (p.x > MLI_MAX3(t.v1.x, t.v2.x, t.v3.x)) return(MLI_OUTSIDE);
   if (p.y > MLI_MAX3(t.v1.y, t.v2.y, t.v3.y)) return(MLI_OUTSIDE);
   if (p.z > MLI_MAX3(t.v1.z, t.v2.z, t.v3.z)) return(MLI_OUTSIDE);
   if (p.x < MLI_MIN3(t.v1.x, t.v2.x, t.v3.x)) return(MLI_OUTSIDE);
   if (p.y < MLI_MIN3(t.v1.y, t.v2.y, t.v3.y)) return(MLI_OUTSIDE);
   if (p.z < MLI_MIN3(t.v1.z, t.v2.z, t.v3.z)) return(MLI_OUTSIDE);
...
/* If any vertex is inside the cube, return immediately!     */
   ...
   if ((v1_test = __mli_face_plane(t.v1)) == MLI_INSIDE) return(MLI_INSIDE);
   if ((v2_test = __mli_face_plane(t.v2)) == MLI_INSIDE) return(MLI_INSIDE);
   if ((v3_test = __mli_face_plane(t.v3)) == MLI_INSIDE) return(MLI_INSIDE);

/* If all three vertexes were outside of one or more face-planes, */
/* return immediately with a trivial rejection!                   */

   if ((v1_test & v2_test & v3_test) != 0) return(MLI_OUTSIDE);
...
/* each triangle edge need be tested.                         */
   ...
   if ((v1_test & v2_test) == 0)
      if (__mli_check_line(t.v1,t.v2,v1_test|v2_test) == MLI_INSIDE)
        return(MLI_INSIDE);
   if ((v1_test & v3_test) == 0)
      if (__mli_check_line(t.v1,t.v3,v1_test|v3_test) == MLI_INSIDE)
        return(MLI_INSIDE);
   if ((v2_test & v3_test) == 0)
      if (__mli_check_line(t.v2,t.v3,v2_test|v3_test) == MLI_INSIDE)
        return(MLI_INSIDE);
...
/* Do this for all four diagonals.                                */
   ...
   /* skip parallel diagonals to the plane; division by 0 can occur */
   {
      ...
      if (fabs(hitpp.x) <= 0.5)
         if (__mli_point_triangle_intersection(hitpp,t) == MLI_INSIDE)
            return(MLI_INSIDE);
   }
   if(fabs(denom=(norm.x + norm.y - norm.z))>MLI_TRI_EPS)
   {
      ...
      if (fabs(hitpn.x) <= 0.5)
         if (__mli_point_triangle_intersection(hitpn,t) == MLI_INSIDE)
            return(MLI_INSIDE);
   }
   if(fabs(denom=(norm.x - norm.y + norm.z))>MLI_TRI_EPS)
   {
      ...
      if (fabs(hitnp.x) <= 0.5)
         if (__mli_point_triangle_intersection(hitnp,t) == MLI_INSIDE)
            return(MLI_INSIDE);
   }
   if(fabs(denom=(norm.x - norm.y - norm.z))>MLI_TRI_EPS)
   {
      ...
      if (fabs(hitnn.x) <= 0.5)
         if (__mli_point_triangle_intersection(hitnn,t) == MLI_INSIDE)
            return(MLI_INSIDE);
   }
