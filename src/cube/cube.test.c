...
CASE("Cube to Orientated-Bounding-Box")
{
        ...
        cube.lower = mli_Vec_set(.0, .0, .0);
        cube.edge_length = 1.;
        a = mli_Cube_to_aabb(cube);
        CHECK(mli_Vec_equal_margin(a.lower, cube.lower, 1e-6));
        CHECK(mli_Vec_equal_margin(a.upper, mli_Vec_set(1., 1., 1.), 1e-6));
}
...
CASE("mli_Cube_outermost_cube")
{
        struct mli_AABB a;
        struct mli_Cube cube;
        a.lower = mli_Vec_set(.0, .0, .0);
        a.upper = mli_Vec_set(1., 2., 3.);
...
CASE("mli_AABB_center")
{
        ...
        aabb.lower = mli_Vec_set(.5, .5, .5);
        aabb.upper = mli_Vec_set(1.5, 1.5, 1.5);
...
CASE("mli_Vec_overlap_aabb, upper, and lower cases")
{
        struct mli_Vec low = {0., 0., 0.};
        struct mli_Vec upp = {2., 2., 2.};
        CHECK(mli_Vec_overlap_aabb(mli_Vec_set(0., 0., 0.), low, upp));
        CHECK(mli_Vec_overlap_aabb(mli_Vec_set(0., 0., 1.), low, upp));
        CHECK(mli_Vec_overlap_aabb(mli_Vec_set(0., 1., 1.), low, upp));
        CHECK(mli_Vec_overlap_aabb(mli_Vec_set(2., 2., 2.), low, upp));
        CHECK(mli_Vec_overlap_aabb(mli_Vec_set(1., 2., 2.), low, upp));
        CHECK(mli_Vec_overlap_aabb(mli_Vec_set(1., 1., 2.), low, upp));
}

CASE("mli_Triangle_has_overlap_aabb")
{
        struct mli_AABB aabb;
        aabb.lower = mli_Vec_set(0., 0., 0.);
        aabb.upper = mli_Vec_set(2., 2., 2.);
