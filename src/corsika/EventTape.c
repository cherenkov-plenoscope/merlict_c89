...
int mliEventTapeReader_read_runh(struct mliEventTapeReader *tio, float *runh)
{
        ...
        chk_msg(mtl_cstr_match_templeate(
...
int mliEventTapeReader_read_evth(struct mliEventTapeReader *tio, float *evth)
{
        ...
        if (!mtl_cstr_match_templeate(tio->tarh.name, match, 'd')) {
                return 0;
        }
        chk_msg(mtl_cstr_nto_uint64(
                        &path_event_number,
        ...
        const struct mliEventTapeReader *tio)
{
        ...
        return mtl_cstr_match_templeate(tio->tarh.name, match, 'd');
}
