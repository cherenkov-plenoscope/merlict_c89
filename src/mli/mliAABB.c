...
{
        struct mliAABB c;
        c.lower.x = MTL_MATH_MIN2(a.lower.x, b.lower.x);
        c.lower.y = MTL_MATH_MIN2(a.lower.y, b.lower.y);
        c.lower.z = MTL_MATH_MIN2(a.lower.z, b.lower.z);
        c.upper.x = MTL_MATH_MAX2(a.upper.x, b.upper.x);
        c.upper.y = MTL_MATH_MAX2(a.upper.y, b.upper.y);
        c.upper.z = MTL_MATH_MAX2(a.upper.z, b.upper.z);
        return c;
}
...
{
        chk_msg(!MTL_MATH_IS_NAN(a.lower.x), "aabb.lower.x is 'nan'.");
        chk_msg(!MTL_MATH_IS_NAN(a.lower.y), "aabb.lower.y is 'nan'.");
        chk_msg(!MTL_MATH_IS_NAN(a.lower.z), "aabb.lower.z is 'nan'.");

        chk_msg(!MTL_MATH_IS_NAN(a.upper.x), "aabb.upper.x is 'nan'.");
        chk_msg(!MTL_MATH_IS_NAN(a.upper.y), "aabb.upper.y is 'nan'.");
        chk_msg(!MTL_MATH_IS_NAN(a.upper.z), "aabb.upper.z is 'nan'.");
