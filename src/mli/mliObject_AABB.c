        ...
        const struct mliHomTra local2root)
{
        ...
        for (face_idx = 0; face_idx < obj->num_faces; face_idx++) {
                ...
                aabb.lower.x = MTL_MATH_MIN2(aabb.lower.x, a_root.x);
                aabb.lower.y = MTL_MATH_MIN2(aabb.lower.y, a_root.y);
                aabb.lower.z = MTL_MATH_MIN2(aabb.lower.z, a_root.z);

                aabb.upper.x = MTL_MATH_MAX2(aabb.upper.x, a_root.x);
                aabb.upper.y = MTL_MATH_MAX2(aabb.upper.y, a_root.y);
                aabb.upper.z = MTL_MATH_MAX2(aabb.upper.z, a_root.z);

                aabb.lower.x = MTL_MATH_MIN2(aabb.lower.x, b_root.x);
                aabb.lower.y = MTL_MATH_MIN2(aabb.lower.y, b_root.y);
                aabb.lower.z = MTL_MATH_MIN2(aabb.lower.z, b_root.z);

                aabb.upper.x = MTL_MATH_MAX2(aabb.upper.x, b_root.x);
                aabb.upper.y = MTL_MATH_MAX2(aabb.upper.y, b_root.y);
                aabb.upper.z = MTL_MATH_MAX2(aabb.upper.z, b_root.z);

                aabb.lower.x = MTL_MATH_MIN2(aabb.lower.x, c_root.x);
                aabb.lower.y = MTL_MATH_MIN2(aabb.lower.y, c_root.y);
                aabb.lower.z = MTL_MATH_MIN2(aabb.lower.z, c_root.z);

                aabb.upper.x = MTL_MATH_MAX2(aabb.upper.x, c_root.x);
                aabb.upper.y = MTL_MATH_MAX2(aabb.upper.y, c_root.y);
                aabb.upper.z = MTL_MATH_MAX2(aabb.upper.z, c_root.z);
        }
        return aabb;
}
