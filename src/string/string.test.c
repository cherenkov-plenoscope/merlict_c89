...
CASE("mtl_String_rfind")
{
        ...
        CHECK(mtl_String_from_cstr_fromat(&str, ""));
        CHECK(mtl_String_find(&str, 'a') == -1);
        CHECK(mtl_String_rfind(&str, 'a') == -1);

        CHECK(mtl_String_from_cstr_fromat(&str, "a"));
        CHECK(mtl_String_find(&str, 'a') == 0);
        CHECK(mtl_String_rfind(&str, 'a') == 0);

        CHECK(mtl_String_from_cstr_fromat(&str, "abc"));
        CHECK(mtl_String_find(&str, 'a') == 0);
        CHECK(mtl_String_rfind(&str, 'a') == 0);

        CHECK(mtl_String_from_cstr_fromat(&str, "123abc"));
...
CASE("mtl_String_ends_with")
{
        ...
        CHECK(mtl_String_from_cstr_fromat(&one, "123"));
        CHECK(mtl_String_from_cstr_fromat(&two, ""));
        CHECK(mtl_String_ends_with(&one, &two));

        CHECK(mtl_String_from_cstr_fromat(&one, ""));
        CHECK(mtl_String_from_cstr_fromat(&two, ""));
        CHECK(mtl_String_ends_with(&one, &two));

        CHECK(mtl_String_from_cstr_fromat(&one, ""));
        CHECK(mtl_String_from_cstr_fromat(&two, "123"));
        CHECK(!mtl_String_ends_with(&one, &two));
        ...
        CHECK(mtl_String_from_cstr_fromat(&one, "my_file.json"));
        CHECK(mtl_String_from_cstr_fromat(&two, ".json"));
        CHECK(mtl_String_ends_with(&one, &two));

        CHECK(mtl_String_from_cstr_fromat(&one, "my_file.json.stuff"));
        CHECK(mtl_String_from_cstr_fromat(&two, ".json"));
        CHECK(!mtl_String_ends_with(&one, &two));
...
CASE("mtl_String_starts_with")
{
        ...
        CHECK(mtl_String_from_cstr_fromat(&one, "my_file.json"));
        CHECK(mtl_String_from_cstr_fromat(&two, "my_file"));
        CHECK(mtl_String_starts_with(&one, &two));

        CHECK(mtl_String_from_cstr_fromat(&one, "my_file.json.stuff"));
        CHECK(mtl_String_from_cstr_fromat(&two, "my_file.json"));
        CHECK(mtl_String_starts_with(&one, &two));

        CHECK(mtl_String_from_cstr_fromat(&one, "functions/hans.csv"));
        CHECK(mtl_String_from_cstr_fromat(&two, "functions/"));
        CHECK(mtl_String_starts_with(&one, &two));
...
CASE("mtl_String_has_prefix_suffix")
{
        ...
        CHECK(mtl_String_from_cstr_fromat(&str, ""));
        CHECK(mtl_String_from_cstr_fromat(&pre, ""));
        CHECK(mtl_String_from_cstr_fromat(&suf, ""));
        ...
        CHECK(mtl_String_from_cstr_fromat(&str, "abc"));
        CHECK(mtl_String_from_cstr_fromat(&pre, "a"));
        CHECK(mtl_String_from_cstr_fromat(&suf, "c"));
        CHECK(mtl_String_has_prefix_suffix(&str, &pre, &suf));

        CHECK(mtl_String_from_cstr_fromat(&str, "abc"));
        CHECK(mtl_String_from_cstr_fromat(&pre, "a"));
        CHECK(mtl_String_from_cstr_fromat(&suf, "d"));
        CHECK(!mtl_String_has_prefix_suffix(&str, &pre, &suf));

        CHECK(mtl_String_from_cstr_fromat(&str, "_abc"));
        CHECK(mtl_String_from_cstr_fromat(&pre, "a"));
        CHECK(mtl_String_from_cstr_fromat(&suf, "c"));
        CHECK(!mtl_String_has_prefix_suffix(&str, &pre, &suf));
...
CASE("mtl_String_copy")
{
        ...
        CHECK(mtl_String_from_cstr_fromat(&src, "abb"));
        CHECK(mtl_String_copy(&dst, &src));
        CHECK(0 == strcmp(dst.array, "abb"));

        CHECK(mtl_String_from_cstr_fromat(&src, ""));
        CHECK(mtl_String_copy(&dst, &src));
...
CASE("mtl_String_strip")
{
        ...
        CHECK(mtl_String_from_cstr_fromat(&src, "   a b  b  "));
        CHECK(mtl_String_strip(&src, &dst));
...
CASE("mtl_String_copyn")
{
        ...
        CHECK(mtl_String_from_cstr_fromat(&src, "123456789.json"));
