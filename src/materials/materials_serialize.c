...
int mli_Materials_to_io(const struct mli_Materials *self, struct mli_IO *f)
{
        ...
        chk_IO_write(&self->surfaces2.size, sizeof(uint64_t), 1u, f);
        chk_IO_write(&self->media2.size, sizeof(uint64_t), 1u, f);
        chk_IO_write(&self->layers2.size, sizeof(uint64_t), 1u, f);
        ...
        for (i = 0; i < self->surfaces2.size; i++) {
                chk(mli_BoundaryLayer_Surface_to_io(
                        &self->surfaces2.array[i], f));
        }
        for (i = 0; i < self->media2.size; i++) {
                chk(mli_BoundaryLayer_Medium_to_io(&self->media2.array[i], f));
        }
        for (i = 0; i < self->layers2.size; i++) {
                chk(mli_BoundaryLayer2_to_io(&self->layers2.array[i], f));
        }
...
int mli_Materials_from_io(struct mli_Materials *self, struct mli_IO *f)
{
        ...
        for (i = 0; i < self->media2.size; i++) {
                chk(mli_BoundaryLayer_Medium_from_io(
                        &self->media2.array[i], f));
        }
        for (i = 0; i < self->layers2.size; i++) {
                chk(mli_BoundaryLayer2_from_io(&self->layers2.array[i], f));
        }
