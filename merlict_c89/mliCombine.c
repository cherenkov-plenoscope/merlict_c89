        ...
        const struct mliOBB obb)
{
        ...
        const struct mliObject *obj_ptr = &combine->scenery->resources.objects[object_idx];
        const struct mliHomTraComp local2root_comp = combine->scenery->robject2root[robject_idx];

        return mliObject_has_overlap_obb(
                obj_ptr,
                local2root_comp,
                obb);
}
