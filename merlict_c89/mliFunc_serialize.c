...
int mliFunc_fwrite(const struct mliFunc *func, FILE *f)
{
        ...
        mli_check(mliMagicId_set(&magic, "mliFunc"));
        mli_check_fwrite(&magic, sizeof(struct mliMagicId), 1u, f);

        mli_check_fwrite(&func->num_points, sizeof(uint32_t), 1u, f);
        mli_check_fwrite(func->x, sizeof(double), func->num_points, f);
        mli_check_fwrite(func->y, sizeof(double), func->num_points, f);
        return 1;
error:
        return 0;
}
