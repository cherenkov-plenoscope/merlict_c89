...
int mliObject_has_valid_vertices(const struct mliObject *obj)
{
        ...
        for (i = 0; i < obj->num_vertices; i++) {
                mli_check_message(!MLI_IS_NAN(obj->vertices[i].x), "X is 'nan'.");
                mli_check_message(!MLI_IS_NAN(obj->vertices[i].y), "Y is 'nan'.");
                mli_check_message(!MLI_IS_NAN(obj->vertices[i].z), "Z is 'nan'.");
        }
        return 1;
error:
        ...
        const double epsilon)
{
        ...
        for (i = 0; i < obj->num_vertex_normals; i++) {
                double norm;
                mli_check_message(!MLI_IS_NAN(obj->vertex_normals[i].x), "X is 'nan'.");
                mli_check_message(!MLI_IS_NAN(obj->vertex_normals[i].y), "Y is 'nan'.");
                mli_check_message(!MLI_IS_NAN(obj->vertex_normals[i].z), "Z is 'nan'.");
