        ...
        const struct mliMaterials *b)
{
        ...
        for (i = 0; i < a->num_media; i++) {
                chk_msg(mliMedium_equal(&a->media[i], &b->media[i]), "Different Medium.");
                chk_msg(mliName_equal(&a->medium_names[i], &b->medium_names[i]), "Different medium-name.");
        }
        return 1;
error:
        ...
        const struct mliMaterials *b)
{
        ...
        for (i = 0; i < a->num_surfaces; i++) {
                chk_msg(mliSurface_equal(&a->surfaces[i], &b->surfaces[i]),
                        "Different Surface.");
                chk_msg(mliName_equal(&a->surface_names[i], &b->surface_names[i]),
...
int mliMaterials_equal(
        ...
        const struct mliMaterials *b)
{
        chk_msg(a->default_medium == b->default_medium, "Different default_medium.");
        chk_msg(_mliMaterials_media_equal(a, b), "Different media.");
        chk_msg(_mliMaterials_surfaces_equal(a, b), "Different surfaces.");
        chk_msg(_mliMaterials_boundary_layers_equal(a, b), "Different boundary_layers.");
        return 1;
error:
        return 0;
}
