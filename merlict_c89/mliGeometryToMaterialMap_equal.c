...
{
        uint32_t i = 0u;
        mli_check(a->num_robjects == b->num_robjects,
                "Expected num_robjects to be equal.");
        mli_check(a->total_num_boundary_layers == b->total_num_boundary_layers,
                "Expected total_num_boundary_layers to be equal.");
        for (i = 0u; i < a->total_num_boundary_layers; i++) {
                mli_check(a->boundary_layers[i] == b->boundary_layers[i],
