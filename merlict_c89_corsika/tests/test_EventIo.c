...
CASE("EventIoHeader_fails_wrong_sync_marker")
{
        ...
        CHECK(fin);
        ftmp = fopen(
                "merlict_c89_corsika/tests/resources/telescope.dat-bad-sync.tmp",
                "wb"
        );
        CHECK(ftmp);
        for (i = 0; i < 100; i++) {
...
CASE("EventIoHeader_fails_empty_file")
{
    ...
    int tmp;
    struct mliEventIoHeader my_header;
    fempty = fopen("merlict_c89_corsika/tests/resources/telescope.dat", "rb");
    CHECK(fempty);
    tmp = getc(fempty);
    while (tmp != EOF) {
       tmp = getc(fempty);
...
CASE("EventIoFile_telescope_dat__check_tel_pos")
{
        ...
        CHECK(mliEventIoRun_malloc(
                &run,
                "merlict_c89_corsika/tests/resources/telescope.dat"));
...
CASE("EventIoFile_telescope_dat__mmcs_run_header")
{
        ...
        CHECK_MARGIN(-2.7, runh[MLI_CORSIKA_RUNH_SLOPE_OF_ENERGY_SPECTRUM], 1e-6);
        ...
        CHECK_MARGIN(220000., runh[MLI_CORSIKA_RUNH_NUM_OBSERVATION_LEVELS + 1], 1e-6);
        mliEventIoRun_free(&run);
}
...
CASE("EventIoFile_telescope_dat__event_header")
{
        ...
        CHECK_MARGIN(-6589.96044922, event.telescope_offsets.array[0].yoff, 1e-6);
...
CASE("EventIoFile_telescope_dat__photon_bundle_size")
{
        ...
        for (int i = 0; i < event.photon_bunches.size; i++) {
            struct mliCorsikaPhotonBunch bunch = event.photon_bunches.array[i];
...
CASE("EventIoFile_telescope_dat__photon_bundle_values")
{
        ...
        for (int j = 2; j < 5; j++) {
                ...
                CHECK_MARGIN(b.z_emission_cm, some[j].z_emission_cm, 10);
