...
{
        struct mliTarHeader tarinfo = mliTarHeader_init();
        ...
        mli_check(tarinfo.size == 273*sizeof(float),
            "Expected raw run-header to have size 273*sizeof(float).");
        mli_check(
            mliTar_read_data(&run->tar, (void *)&(run->header), tarinfo.size),
        ...
        struct mliTarIoRun *run)
{
        ...
        mli_check(strlen(info_header.name) == EVENT_HEADER_PATH_LENGTH,
            "Expected length of evth path to be "
            "strlen('012345678.evth.float32').");

        mli_check(
            strcmp(
                    &info_header.name[PATH_NUM_DIGITS],
                    EVENT_HEADER_SUFFIX
            ) == 0,
        ...
        mli_check(strlen(info_bunches.name) == BUNCH_PATH_LENGTH,
        ...
        mli_check(photon_bunches_event_id == evth_event_id,
