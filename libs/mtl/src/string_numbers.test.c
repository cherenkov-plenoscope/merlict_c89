...
CASE("mtl_String_nto_int64")
{
        ...
        /* 0 */
        mtl_String_mallocf(&s, "0");
        CHECK(mtl_String_to_int64(&i, &s, 10));
        CHECK(i == 0);

        /* INT_MAX. */
        mtl_String_mallocf(&s, "%d", INT_MAX);
        CHECK(mtl_String_to_int64(&i, &s, 10));
        CHECK(i == INT_MAX);

        /* INT_MIN. */
        mtl_String_mallocf(&s, "%d", INT_MIN);
        CHECK(mtl_String_to_int64(&i, &s, 10));
        CHECK(i == INT_MIN);

        /* Leading and trailing space. */
        mtl_String_mallocf(&s, " 1");
        CHECK(!mtl_String_to_int64(&i, &s, 10));
        mtl_String_mallocf(&s, "1 ");
        CHECK(!mtl_String_to_int64(&i, &s, 10));
        ...
        /* Trash characters. */
        mtl_String_mallocf(&s, "a10 ");
        CHECK(!mtl_String_to_int64(&i, &s, 10));
        mtl_String_mallocf(&s, "10a ");
        CHECK(!mtl_String_to_int64(&i, &s, 10));
        CHECK(mtl_String_nto_int64(&i, &s, 10, 2));
        CHECK(i == 10);

        /* long overflow */
        mtl_String_mallocf(&s, "%ld0", LONG_MAX);
        CHECK(!mtl_String_to_int64(&i, &s, 10));

        /* long underflow */
        mtl_String_mallocf(&s, "%ld0", LONG_MIN);
        CHECK(!mtl_String_to_int64(&i, &s, 10));
