...
int mliAvlDict_update__(const struct mliAvlNode* node, struct mliAvlDict* out)
{
        ...
        chk_msg(
                mliAvlDict_set(out, node->key, node->value),
                "Failed to insert key/value into destination dict while updating."
        );
...
int mliAvlDict_grow(struct mliAvlDict* dict)
{
        ...
        chk_msg(
                mliAvlDict_malloc(&tmp, new_capacity),
                "Failed to malloc bigger tmp dict in order to grow."
        );

        /* copy nodes */
        chk_msg(
                mliAvlDict_update__((const struct mliAvlNode*)dict->tree.root, &tmp),
...
int mliAvlDict_insert(struct mliAvlDict* dict, const int64_t key, const int64_t value)
{
        int insert_rc;
        ...
        insert_rc = mliAvlTree_insert(&dict->tree, (struct mliAvl*)(back_node));
...
int mliAvlDict_set(struct mliAvlDict* dict, const int64_t key, const int64_t value)
{
        ...
        if (nn != NULL) {
                ...
                chk_msg(
                        mliAvlDict_insert(dict, key, value),
...
int mliAvlDict_shrink(struct mliAvlDict* dict)
{
        ...
        chk_msg(
                mliAvlDict_update__((const struct mliAvlNode*)dict->tree.root, &tmp),
                "Failed to copy nodes over to smaller tmp dict while shrinking."
        );
